...
<div class="d-flex border border-success p-3">
	<div>
		...
		for (int i = 0; i < musicList.size(); i++) {
			if(search == null && (listTitle.equals(musicList.get(i).get("title"))) || (search != null && search.equals(musicList.get(i).get("title")))) {
				
		
		%>
		<img src="<%=musicList.get(i).get("thumbnail")%>" alt="앨범 표지" width="170px" height="200px">
	</div>
	<div class="albumInfo ml-2">
		<div class="display-4"><%=musicList.get(i).get("title")%></div>
		<h6 class="font-weight-bold text-success mt-2"><%=musicList.get(i).get("singer")%></h6 >
		<div class="d-flex mt-3">
			...
			<div>
				<div class="text-secondary"><%=musicList.get(i).get("album")%></div>
				<%
				String time = String.valueOf(musicList.get(i).get("time"));
				time = time.substring(0,1) + " : " + time.substring(1,3);
				%>
				<div><%= time %></div>
				<div class="text-secondary"><%=musicList.get(i).get("composer")%></div>
				<div class="text-secondary"><%=musicList.get(i).get("lyricist")%></div>
			</div>
		</div>
	</div>	
		<%
